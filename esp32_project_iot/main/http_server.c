...
esp_err_t http_server_OTA_update_handler(httpd_req_t *req)
{
	...
	do
	{
		...
		printf("http_server_OTA_update_handler: recv_len %d \n", recv_len);
		...
		if (!is_req_body_started)
		{
			...
			else
			{
				...
				printf("http_server_OTA_update_handler: Writing to stm32 flash \n");
			}
			...
			//Send OTA Header
			meta_info ota_info;
			ota_info.package_size = content_length-(body_start_p - ota_buff);
			...
			ex = send_ota_data( comport, body_start_p, body_part_len );
			if( ex < 0 )
			{
				printf("send_ota_data Err1 \n");
				return ex;
			}
		...
		else
		{
			...
			ex = send_ota_data( comport, ota_buff, recv_len );
			if( ex < 0 )
			{
				printf("send_ota_data Err2 \n");
				return ex;
			}
			content_received += recv_len;
			printf("http_server_OTA_update_handler: OTA RX: %d of %d\n", content_received, content_length);
			if(recv_len<sizeof(ota_buff))
			{
				printf("Ket thuc qua trinh send DATA \n");
